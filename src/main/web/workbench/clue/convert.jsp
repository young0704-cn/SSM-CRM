...
<script type="text/javascript">
	$(function(){
		...
		$("#submit-activity-btn").click(function (){
			if ($("input[name=activity]:checked").length===1){
				id=$("input[name=activity]:checked").val()
				...
				if ($.trim($("#aname").val()).length!==0){
					$.ajax({
						...
						success:function (result_data) {
							//{List<Activity>:{Activity1,Activity2,Activity3},success:true/false,msg:"xxx"}
							html=""
							if (result_data.success){
								$.each(result_data.activityList,function (i,n) {
									html+='<tr><td><input type="radio" name="activity" value="'+n.id+'"/></td>';
									html+='<td id="'+n.id+'">'+n.name+'</td>';
									html+='<td>'+n.startDate+'</td>';
									html+='<td>'+n.endDate+'</td>';
									html+='<td>'+n.owner+'</td></tr>';
								})
								$("#activity-list-tbody").html(html)
							}else {
		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" id="submit-activity-btn">提交</button>
				</div>
			</div>
		</div>
	</div>
